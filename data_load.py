...
def _load_data(fpath, maxlen1, maxlne2):
    ...
    :param maxlne2: evidence max length
    ...
    for data in df:
        question = data[1]
        evidence = data[0]
        if len(question) > maxlen1 or len(evidence) > maxlne2:
            continue
        label = data[3]
        start = data[4]
        end = data[5]
...
def _generator_fn(questions, evidences, labels, vocab_fpath):
    ...
    for question, evidence, label in zip(questions, evidences, labels):
        tokens1 = [token2idx.get(token, token2idx['<unk>']) for token in list(question)]
        tokens2 = [token2idx.get(token, token2idx['<unk>']) for token in list(evidence)]
...
def _input_fn(questions, evidences, labels, vocab_fpath, batch_size, gpu_nums, maxlen1, maxlen2, shuffle=False):
    ...
    dataset = dataset.padded_batch(batch_size*gpu_nums, shapes, paddings).prefetch(1)
